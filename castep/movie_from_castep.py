...
# Added reading MD summary data Nov 2022.
#
...
#
# Read in MD Data information
#
        if ( nwords == 4 ):
            if ( 'MD' in linesplit[1] and 'Data' in linesplit[2] ):
                print("Reading MD_data section!")
                md_datalines=10
#
        if ( md_datalines >= 0 ):
            md_datalines= md_datalines-1
            if nwords > 2:
                if ('time' in linesplit[1]):
                    tim_list.append(float(linesplit[3]))
                elif ('Potential' in linesplit[1]):
                    pot_list.append(float(linesplit[3]))
                elif ('Kinetic' in linesplit[1]):
                    kin_list.append(float(linesplit[3]))
                elif ('Total' in linesplit[1]):
                    tot_list.append(float(linesplit[3]))
                elif ('Hamilt' in linesplit[1]):
                    ham_list.append(float(linesplit[3]))
                elif ('Temper' in linesplit[1]):
                    tempr_list.append(float(linesplit[2]))
#
#
...
#    print(cell)           
    ...
    return atoms, tim_list, pot_list, kin_list, tot_list, ham_list, tempr_list
...
print(f"MD data summary file         : %s" % csv_file )
print(f"------------------------------------------------------------")
#
...
atoms, tim_list, pot_list, kin_list, tot_list, ham_list, tempr_list=read_castep(file_pntr, traj)
...
# Deal with MD data
#
file_pntr=open(csv_file, 'wt')
titles="time / ps, Potential Energy / eV, Kinetic Energy / eV, Total Energy / eV, Hamiltonian Energy / eV, Temperature / K\n"
file_pntr.write(titles)

for ilist in range(0,len(tim_list)):
    data_list=f"%10.6f,%10.6f,%10.6f,%10.6f,%10.6f,%10.6f\n" % (tim_list[ilist], pot_list[ilist], kin_list[ilist], tot_list[ilist], ham_list[ilist], tempr_list[ilist])
    file_pntr.write(data_list)
