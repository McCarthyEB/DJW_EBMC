...
#
geom_file = sys.argv[1]                                           # Directory where we will do the calculations
splstr=geom_file.split('.')
stem=splstr[0]
castep_cellfile="%s_new.cell" % stem
ciffile="%s_new.cif" % stem
poscarfile="%s_new.poscar" % stem
#
print(f"------------------------------------------------------------")
print(f"geom file to convert          : %s" % geom_file )
print(f"Stem                         : %s" % stem     )
...
write(castep_cellfile, atoms ,format='castep-cell',positions_frac=True)
write(ciffile, atoms ,format='cif')
write(poscarfile, atoms ,format='vasp')
