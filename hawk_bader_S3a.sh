#!/bin/bash --login
#SBATCH -o BadS3a.o         # Job output file
#SBATCH -e BadS3a.e         # Job error file
#SBATCH -J BadS3a            # Job name
...
#
     ...
     cp POS*   "$results_dir"
#
# Carry out Bader analysis
#
     echo "Combining files for Bader" >>  $LOG_FILE
     if [ ! -e AECCAR0 ]; then
       echo "ERROR no AECCAR0 file. Make sure VASP job used LAECHG=.TRUE."  >> $LOG_FILE
     fi

     if [ ! -e AECCAR2 ]; then
       echo "ERROR no AECCAR2 file. Make sure VASP job used LAECHG=.TRUE."  >> $LOG_FILE
     fi

     $HOME/bin/vtstscripts/chgsum.pl AECCAR0 AECCAR2

     echo "Running Bader analysis" >>  $LOG_FILE  

     if [ ! -e CHGCAR_sum ]; then
       echo "ERROR no CHGCAR_sum file. Something is wrong with the chgsum step"  >> $LOG_FILE
     fi

     bader CHGCAR -ref CHGCAR_sum >> $LOG_FILE
#
     cp ACF.dat "$results_dir"
     cp AVF.dat "$results_dir"
     cp BCF.dat "$results_dir"
#
done
#
# clean scratch
