...
if PROG == "AIMS":
  aims_filestem= "geometry_%s.in" % sys.argv[5]                        # stem to use for geometry.in files output at scan steps
...
print(f"Type of structure file         : %s" % GEOM )
#print(f"Hessian estimate in            : %s" % machine )
#
...
#
  ...
  if GEOM == "ZnO.traj":
    traj=read('ZnO.traj')
    last_frame=traj[-1]
    write('geometry.in', traj, format='aims')
    atoms=read('geometry.in')
#  if os.path.exists('hessian.aims'):
 #   hessian_data = ('hessian.aims')
  elif GEOM == "ZnO_bulk.cif":
     atoms=read("ZnO_bulk.cif")
  elif GEOM == "geometry.in":
    atoms=read('geometry.in')
...
# Change some FHI-aims calculation settings
  ...
  sockets_calc, fhi_calc = get_aims_and_sockets_calculator(dimensions=2,
             #Added per aims manual: anything with an appreciable bandgap should have a charge mixing parameter of 0.2, instead of the metals default of 0.02
             charge_mix_param=0.2,
             check_cpu_consistency=".true.", 
             xc_pre=['pbe', '15'], #we do here 10 steps with PBE to stabilize the system
             override_warning_libxc="true", #MBEEF is not impplemented directly with FHI, for that we need to call libxc
             xc='libxc MGGA_X_MBEEF+GGA_C_PBE_SOL', #MBEEF
             #spin='none', #Can comment out as already implemented in this version of aims_calculator.py
             k_grid=(3,3,1),
             #relativistic=('atomic_zora', 'scalar'), #already implemented in this version of aims_calculator.py
#             compute_forces=".true.",
...
if debug:
   ...
   #
   dynamics = BFGS(atoms, trajectory='ZnO_new.traj', restart='qn.pckl')  #Put optimisation back in
   dynamics.run(fmax=0.01)
   energy=atoms.get_potential_energy()
   
   write('geometry_next_step.in', atoms, format='aims')
...
#          csv_file.close()
   ...
   print("Calculation finished! Closing socket connection...")
#
   sockets_calc.close()
