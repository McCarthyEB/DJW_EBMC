...
if [[ $MACHINE == "young" ]]; then
  echo "I am Young"
  JOBID=$JOB_ID
  JOBNAME=$JOB_NAME
  sub_dir="optrun.${JOBID}"
...
#FHI-aims set up
	...
	if [[ $MACHINE == "archer2" ]]; then
		...
		export ASE_AIMS_COMMAND="srun --distribution=block:block --hint=nomultithread $executable"
	       	export AIMS_SPECIES_DIR="/work/e05/e05-files-log/shared/software/fhi-aims/species_defaults/defaults_2020/light"
		...
		export PATH=$PYTHONUSERBASE/bin:$PATH
	       	export PYTHONPATH=$PYTHONUSERBASE/lib/python3.8/site-packages:$PYTHONPATH
	       	export PYTHONPATH=$PYTHONUSERBASE/lib/python3.8/site-packages/carmm:$PYTHONPATH
		#Pick up local version of carmm!
	       	#export PYTHONPATH=$PYTHONUSERBASE/bin/carmm:$PYTHONPATH
	       	#export PYTHONPATH=$PYTHONUSERBASE/bin/ase:$PYTHONPATH
                echo "The current pythonpath is " $PYTHONPATH
        

        elif [[ $MACHINE == "young" ]]; then
                
		echo "Importing modules and setting ASE_AIMS_command for Young"
                # Using default 2018/3 for now. The below worked on Thomas
                #module purge
                module unload python3/recommended
                #module load gcc-libs
                module switch -f compilers/intel/2018/update3 compilers/intel/2019/update4
                module switch -f mpi/intel/2018/update3/intel mpi/intel/2019/update4/intel
                echo "Setting launch_dir and job_id"
		...
		JOBID=$JOBNAME
                ...
                #export VERSION=201009
                #export VERSION=210309
                export VERSION=220619
                #export VERSION=220214
          ...
          #      export executable=/home/mmm1004/progs/aims.$VERSION.scalapack.mpi.x
           ...
           #     export AIMS_SPECIES_DIR="/home/mmm0170/Software/fhi-aims/species_defaults/defaults_2020/tight"
                ...
                #export PYTHONPATH=/home/mmm0170/Software/carmm:$PYTHONPATH
                export PYTHONPATH=~/bin/carmm:$PYTHONPATH
                echo "Pythonpath is"
                echo $PYTHONPATH
...
#
echo LAUNCH_DIR set to $LAUNCH_DIR really
cd $LAUNCH_DIR
#
#export OMP_NUM_THREADS=1
#export I_MPI_ADJUST_ALLTOALLV=2
...
#
    ...
    export work_dir=$PWD
#    export work_dir=/home/mmm1004/Scratch/workspace/$
...
#
       ...
       elif  [[ $struct_name == *".cif" ]]; then
          cp "$full_struct" ZnO_bulk.cif
       elif  [[ $struct_name == *".in" ]]; then
          cp "$full_struct" geometry.in
       elif  [[ $struct_name == *".traj" ]]; then
          cp "$full_struct" ZnO.traj
else
...
#If we have a qn.pckl file, we'll include it here too!
     if [ -e "$full_hess" ]; then 
         echo Copying qn.pckl file $full_hess to $jobwork_dir >> $LOG_FILE
...
#
     ...
     echo python3 $ASE_SCRIPT $jobwork_dir $JOBID $LAUNCH_DIR $struct_dir $sub_dir  \
                    $MACHINE $zfix $hess_name $struct_name $ind1 $ind2 $ind3 > "$LAUNCH_DIR"/"$struct_dir"/"$sub_dir"/"$ASE_SCRIPT"_"$JOBNAME".out >> $LOG_FILE
#
#  run the ase script
#
     if [[ $MACHINE == "young" ]]; then
        time python3 $ASE_SCRIPT $jobwork_dir $CORES_PER_TASK $JOBID $LAUNCH_DIR $struct_dir $sub_dir $MACHINE $zfix $full_hess $struct_name $ind1 $ind2 $ind3 \
                                    > "$LAUNCH_DIR"/"$struct_dir"/"$sub_dir"/"$ASE_SCRIPT"_"$JOBID".out
     else
        python3 $ASE_SCRIPT $jobwork_dir $CORES_PER_TASK $JOBID $LAUNCH_DIR $struct_dir $sub_dir $MACHINE $zfix $full_hess $struct_name $ind1 $ind2 $ind3\
...
#
        ...
        #cp * "$results_dir"     
        if [[ $MACHINE == "young" ]]; then
          mv "$LAUNCH_DIR"/"$JOBNAME".o"$JOB_ID" "$LAUNCH_DIR"/"$JOBNAME".e$JOB_ID "$LAUNCH_DIR"/"$ASE_SCRIPT"_"$JOBID".log "$results_dir"
        elif [[ $MACHINE == "archer2" ]]; then
