...
#
# Read the POSCAR directly to work out the atoms flagged as free
#
fileobj=open("POSCAR",'r+')
lines = fileobj.readlines()
nlines = len(lines)
fileobj.close()
#
print("File has : ", nlines, " lines.")
#
free_atoms=[]
iatom=0
#
...
for iline in range(0,len(lines)):
    ...
    nwords=len(words)
    if (atom_lines and nwords == 0):
        atom_lines=False
#
    if (atom_lines and "T" in words[3]):
        free_atoms.append(iatom)
        print("Free atom found ", iatom)
#
    if (atom_lines):
        iatom += 1
#
    if (nwords == 1 and ("irect" in words[0] or "artes" in words[0])):
        atom_lines=True
#
for iatom in range(0,len(atoms)):
    fix_lab="fixed"
    found=False
    for ifree in range(0,len(free_atoms)):
       if (iatom == free_atoms[ifree]): found=True

    if (found): fix_lab="free"
#
...
# Set to pick up wavefunction and charge density from energy calc.
#
calc.set(istart=1)
atoms.set_calculator(calc)
#
vib = Vibrations(atoms, indices=free_atoms)
#
vib.run()
vib.summary()
vib.write_mode(-1)
