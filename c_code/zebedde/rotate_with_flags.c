...
/*******                                                              *******/
/******* which_mol added to allow multiple adsorbates.                *******/
...
void rotate_with_flags(atom *p_molecule, double *p_axis, double *p_origin,
                       double theta, int *p_flag_list, int num_atoms, int which_mol)
{
...
/*************************************************************/
/**** This is right! p_origin is a pointer to the start of ***/
/**** the origin vector and p_origin_comp is a copy of the ***/
/**** pointer. p_origin_comp is used so we can step through***/
/**** the vector components without losing track of where  ***/
/**** the vector starts! move_molecule needs the vector    ***/
/**** start point passing! Dave Willock note July 1999     ***/
/*************************************************************/
  ...
  *p_origin_comp= (*p_origin_comp) * -1.0;
  ...
  move_molecule( p_molecule, num_atoms, p_origin, -1); 

//  printf("DEBUG>> ..Rotation moved to origin now....\n");
//  p_atom= p_molecule;
...
/************ and if which_mol positive only on that molecule ******/

  used=0;
  for (iloop= 0; iloop <= num_atoms; iloop++)
  {
    p_atom= p_molecule+iloop; 
    p_flag= p_flag_list+iloop;

//    printf("Trying to apply rotation matrix flag %d which_mol %d atom in molecule %d\n", *p_flag, which_mol, p_atom->mol);

// Note that the test of the atoms molecule should now be redundant

    if ( *p_flag && (which_mol < 0 || p_atom->mol==which_mol+1))
      {
...
//if (used > 0) printf("DEBUG>> rotation matrix applied %d times.\n", used);
//else printf("No atoms rotated....\n");
