...
/******* format and no combination rules, i.e. explicit potentials*********/
/******* Dave Willock Feb 2006                                    *********/
/*******                                                          *********/
/******* Intra-molecular capability added August 06               *********/
/******* In the intra case use a pre-assembled list               *********/
/*******                                                          *********/
/**************************************************************************/
...
double non_bond_a_b_none(atom *p_pore, int num_p_atoms,
                         ...
                         double *p_grad, int is_intra, double *p_pos_separations, 
                         double *p_pos_vectors, int is_self)
{
#include "header.h"
atom *p_tatom, *p_patom;

int j,pot_index, this_pair, ipot;
int index_temp_atom,num_interactions; 

double dx,dy,dz,r,r2,r3,r6,r8,r12;
...
/**************************************************************************/
          ...
          while (ipot < 0 && pot_index <= num_potential_types)
            { 
              if (strcmp(potent[pot_index].pot, p_patom->pot)==0 &&
                  strcmp(potent[pot_index].pot2, p_tatom->pot)==0)  
             
                 ipot = 1;
     
              else if (strcmp(potent[pot_index].pot2, p_patom->pot)==0 &&
                       strcmp(potent[pot_index].pot, p_tatom->pot)==0)  
          ...
          if (ipot > 0)
            {
/*              printf("Recognised %s with %s as potential type %d\n",   */
/*                       p_patom->pot, p_tatom->pot, pot_index);         */
...
/******* Catch very close interactions *************************************/
                    if (r2 < 0.01) return 10000;
...
/*               printf("Have %d interactions to consider\n", num_interactions); */
...
/*************************************************************************/
/******* Find potential for this pair of atoms                 ***********/
/*************************************************************************/

                    aij= potent[pot_index].a;
                    bij= potent[pot_index].b;

/*                    printf("For this 12-6 potential using %10.6f and %10.6f\n",    */
/*                          aij, bij);                                               */
