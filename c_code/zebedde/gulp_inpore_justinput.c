/*********************************************************************/
/* gulp_inpore_justinput.c : just produces a GULP input file for     */
/*			template and pore,                           */
/*                       does not carry out minimisation             */
/*         KEJ 14/11/06                                              */
/*  The gulp input file will be labelled _final.gin, and is set up   */
...
gulp_input_count++;
printf("in gulp_inpore_justinput routine-just producing inputi %d- no minimisation!!\n", gulp_input_count);

strcpy(fileroot,root_name);
strcat(fileroot,"_final");
sprintf(tempno, "%d", gulp_input_count);
...
for (i=0;i<=num_atoms;i++)
	...
	if (strcmp(p_OH_elem, (p_atom->pot)) == 0)        {        fprintf(gulp_fp,"O core %12.8f %12.8f %12.8f %12.8f 1 1 1\n",                                                                                                                                              p_atom->x, p_atom->y, p_atom->z, OH_CHARGE);
        }
