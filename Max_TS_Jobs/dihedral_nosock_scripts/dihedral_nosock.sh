...
#RESTART_DIR="/scratch/c.c1809139/56303321/test/optrun.56303321"
RESTART_DIR="no restart"
SOCKETS="no sockets"
#ind1='97'
#ind2='159'
...
# Make sure we pick up local installation of ase
     export GITMODS=/work/e05/e05/$USER/python/DJW_EBMC
     export PYTHONUSERBASE=/work/e05/e05/$USER/.local
     export PATH=$PYTHONUSERBASE/bin:$PATH
     export PYTHONPATH=$PYTHONUSERBASE/lib/python3.8/site-packages:$PYTHONPATH
#
     export PATH=$GITMODS/modules:$PATH
...
#
     ...
     ( 
     shopt -s nullglob 
     if [[ $RESTART_DIR == *"no restart"* ]]; then
	echo "This is a new job"
     else
       vib_files=`ls "$RESTART_DIR"/*.pckl "$RESTART_DIR"/*.json`
          for this_vib in $vib_files; do
            if [ -s $this_vib ]; then
               cp "$this_vib" $jobwork_dir
            fi
          done
     fi
     if [[ $SOCKETS == *"no sockets"* ]]; then
	echo "This calculation is running without sockets"
     fi
)
...
#
     ...
     echo python3 $ASE_SCRIPT $jobwork_dir $JOBID $LAUNCH_DIR $struct_dir $sub_dir  \
                    $MACHINE $zfix $full_hess $struct_name > "$LAUNCH_DIR"/"$struct_dir"/"$sub_dir"/"$ASE_SCRIPT"_"$JOBID".out >> $LOG_FILE
#
#  run the ase script
#
     if [[ $SOCKETS == *"no sockets"* ]]; then
