...
# 3. Request 4 gigabyte of RAM per process (must be an integer)
#$ -l mem=4G

# 5. Set the name of the job.
#$ -N nodef_110

# 6. Select the MPI parallel environment and 40 processes.
#$ -pe mpi 40
...
echo "PBS JOBNAME is" 
echo ${JOBNAME}
echo "Trying PBS JOBID is {JOBID}" 
echo ${JOBID}
echo "Trying PBS JOBID is {PBS_JOBID/.*}" 
echo ${JOBID/.*}
struct_name='geometry.in'
#param_name="silica_standard_TS.param"
sub_dir="optrun.${JOBID}"
zfix="NoFix"
hess_name='hessian.aims'
#ind1='97'
#ind2='159'
	...
	if [[ $MACHINE == "archer2" ]]; then
		...
		JOBID=$JOBNAME
          ...
          #      export executable=/home/mmm1004/progs/aims.$VERSION.scalapack.mpi.x
              ...
              #  export ASE_AIMS_COMMAND="time gerun /home/mmm0170/Software/fhi-aims/bin/aims.$VERSION.scalapack.mpi.x > test_output.log.$JOB_ID"
           #     export AIMS_SPECIES_DIR="/home/mmm0170/Software/fhi-aims/species_defaults/defaults_2020/tight"
                env
                
                #time gerun /home/mmm0170/Software/fhi-aims/bin/aims.$VERSION.scalapack.mpi.x > test_output.log.$JOB_ID
...
#
     ...
     echo python3 $ASE_SCRIPT $jobwork_dir $JOBID $LAUNCH_DIR $struct_dir $sub_dir  \
                    $MACHINE $zfix > "$LAUNCH_DIR"/"$struct_dir"/"$sub_dir"/"$ASE_SCRIPT"_"$JOBNAME".out >> $LOG_FILE
#
#  run the ase script
#
     if [[ $MACHINE == "young" ]]; then
        time python3 $ASE_SCRIPT $jobwork_dir $CORES_PER_TASK $JOBID $LAUNCH_DIR $struct_dir $sub_dir $MACHINE $zfix $full_hess \
                                    > "$LAUNCH_DIR"/"$struct_dir"/"$sub_dir"/"$ASE_SCRIPT"_"$JOBID".out
     else:
        python3 $ASE_SCRIPT $jobwork_dir $CORES_PER_TASK $JOBID $LAUNCH_DIR $struct_dir $sub_dir $MACHINE $zfix $full_hess \
