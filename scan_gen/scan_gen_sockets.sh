...
export CODE='AIMS'
MACHINE='hawk'
ASE_SCRIPT='scan_gen_dist_sockets.py'
struct_list='test'
CORES_PER_TASK=32
#
JOBID=$SLURM_JOBID
...
#
     ...
     echo python3 $ASE_SCRIPT $jobwork_dir $JOBID $LAUNCH_DIR $struct_dir $sub_dir  \
                    $MACHINE $zfix $ind1 $ind2 $hess_name > "$LAUNCH_DIR"/"$struct_dir"/"$sub_dir"/"$ASE_SCRIPT"_"$JOBID".out >> $LOG_FILE
#
#  run the ase script
#
     python3 $ASE_SCRIPT $jobwork_dir $CORES_PER_TASK $JOBID $LAUNCH_DIR $struct_dir $sub_dir $MACHINE $zfix $ind1 $ind2 $hess_name \
